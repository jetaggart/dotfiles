...
Use `trash` instead of `rm` for deleting files. On macOS it's a shell function that moves files to `~/.Trash`. On linux it's `trash-cli` (same `trash` command). Both are recoverable.
</file_management>
